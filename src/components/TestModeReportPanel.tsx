...
Please review this checklist and the current prototype state. Page-level code splitting has been added for large prototype pages. Bundle size should continue to be monitored. Supabase Auth is connected for login flows (signup requires email confirmation). Supabase listings are connected in read-only mode for Home, Search, and Listing Details. Saved listings persistence added for signed-in Supabase users via the saved_listings table. Post Vacancy can save Supabase listing drafts for signed-in poster roles, upload photos to the private listing-photos-pending bucket, and submitted listings become pending_review. Submitted listings are not public until admin approval, at which point a webhook syncs their photos to the public listing-photos bucket. Signed-out users see login prompt. Reports, Contact Events, Dashboard remain prototype-only/local. Suggest structural improvements for the mobile layout, identify any UX gaps in the tenant or poster journeys, and recommend the best technical approach for the next backend phase.
`;
    setReportText(text);
